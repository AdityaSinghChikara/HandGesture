...
WeightFileName = ["Train1.hdf5","bw_4015imgs_weights.hdf5","bw_2510imgs_weights.hdf5","./bw_weight.hdf5","./final_c_weights.hdf5","./semiVgg_1_weights.hdf5","/new_wt_dropout20.hdf5","./weights-CNN-gesture_skinmask.hdf5"]
...
def update(plot,x):
    ...
    #maxi=max(jsonarray,key=jsonarray.get)
    #cv2.putText(plot,maxi,(0,y+5), font , 0.7,(0,255,0),2,1)
   # '''
    global maxi
    global temp
    global t1
    #if(max(jsonarray,key=jsonarray.get,default=' ')!="NOTHING"):
        #maxi=maxi+" "+max(jsonarray,key=jsonarray.get,default=' ')
    #maxi=maxi+" "+max(jsonarray,key=jsonarray.get)
    for items in jsonarray:
        if jsonarray[items]>temp:
            temp=jsonarray[items]
    for items in jsonarray:
        if jsonarray[items]==temp:
            if items!="NOTHING":
                t1[x]=items
            ...
            #maxi=maxi+items
            break
    cv2.putText(plot,maxi,(0,y), font , 0.7,(0,255,0),2,1)
